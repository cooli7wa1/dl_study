...
void platform_set_boot_args_by_atag(unsigned int *ptr)
{
  int i=0, j=0;
    ...
    if(g_is_fast_meta) {
        ...
        if(!g_meta_log_disable)
            tags->u.meta_com.meta_com_id = 4;
        else
            tags->u.meta_com.meta_com_id = 1;
   }
    else
 #endif
   {
  ...
  //----------------
    ...
    tags->u.gz_info.gz_addr = gz_start_addr;
    ptr += tags->hdr.size;
#endif
...
#if CFG_GZ_SUPPORT
    gz_start_addr = ((u32)mblock_reserve_ext(&bootarg.mblock_info, GZ_DRAM_SIZE, GZ_ALIGNMENT, 0x80000000, 0, "gz"));
    if(!gz_start_addr || gz_start_addr != 0x7FE00000) {
        print("%s GZ fatal error...\n", MOD);
        ASSERT(0);
    }
#endif
}
...
bool check_meta_short_pin(void)
{
    ...
    if(!mt_get_gpio_in(GPIO0)) { // detect GPIO pull LOW
        print("Detect META short pin.\n");
        return TRUE;
    }
    else
#endif
