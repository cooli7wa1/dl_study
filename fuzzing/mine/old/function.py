...
def test(images):
    w1 = _get_variable('weights_test1', [192, 32],
                       tf.truncated_normal_initializer(stddev=0.1, dtype=tf.float32))
    b1 = _get_variable('biases_test1', [32], tf.constant_initializer(0.1))
    out1 = tf.nn.relu(tf.matmul(images, w1) + b1, name='layer_test1')
    w2 = _get_variable('weights_test2', [32, 2],
                       tf.truncated_normal_initializer(stddev=0.1, dtype=tf.float32))
    b2 = _get_variable('biases_test2', [2], tf.constant_initializer(0.1))
    # out2 = tf.nn.relu(tf.matmul(out1, w2) + b2, name='layer_test2')
    out2 = tf.matmul(out1, w2) + b2
    return out2
